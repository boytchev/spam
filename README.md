...
- Being added to **email lists** without my consent. The provision of [Unsubscribe] link does not change my opinion. I never use such links.
- Using **texts embedded in images**. This technique is used to bypass anti-spam filters. My email client filters out all images, unless they are from trusted sources. 
- Using **fake email address, domain or IP address**, especially if the public sender differs from the mail delivery route in the message header.
- **Mimicking legitimate sites**. This approach is used to diverge academic traffic (both papers and financial resources) to predators.
- Using **public emails** from gmail.com, yahoo.com, and so on. Serious academic events and publishers must have their own institutional emails.
- Using services for **mass emailing**. It doesn't matter whether some one does this honestly and in good faith. If it is done, I block the whole mail service.
...
- Showing **misleading or custom ranking**, e.g. Impact Factors that are fake (yes, I check Web of Science), claims that something is indexed by Scopus, but it is not found there, using own ranking systems that mimic IF and SJR, but are not.
- Asking me to **review a paper** for a journal or to **join a program committee** of a conference, without establishing a proper contact with me and having my agreement.
